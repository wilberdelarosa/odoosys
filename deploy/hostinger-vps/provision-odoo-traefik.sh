...
echo "==> Levantando base PostgreSQL aislada"
dc up -d db
for attempt in {1..30}; do
  if dc exec -T db pg_isready -U "$ODOO_DB_USER" -d postgres >/dev/null 2>&1; then
    break
  fi
...
dc exec -T db psql -U "$ODOO_DB_USER" -d postgres -c "DROP DATABASE IF EXISTS $ODOO_DATABASE WITH (FORCE);"
dc exec -T db psql -U "$ODOO_DB_USER" -d postgres -c "CREATE DATABASE $ODOO_DATABASE OWNER $ODOO_DB_USER;"

echo "==> Instalando modulos base"
dc run --rm -T odoo odoo -c /etc/odoo/odoo.conf \
  --db_host=db \
...
dc run --rm -T odoo odoo shell -c /etc/odoo/odoo.conf \
  --db_host=db \
  --db_user="$ODOO_DB_USER" \
  --db_password="$ODOO_DB_PASSWORD" \
  -d "$ODOO_DATABASE" <<PY
