...
export class OperationsDashboard extends Component {
    ...
    setup() {
        ...
        this.state = useState({
            ...
            activeSector: localStorage.getItem('ops_active_sector') || 'supermarket',
    ...
    async changeBusinessPreset(sector) {
        this.state.activeSector = sector;
        localStorage.setItem('ops_active_sector', sector);
        
        try {
            const res = await fetch("http://localhost:3000/api/presets/load", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ preset: sector })
            });
            if (res.ok) {
                this.notification.add(`Giro comercial cambiado a ${sector.toUpperCase()} y sincronizado con el Gateway.`, {
                    type: "success",
                    sticky: false,
                });
            } else {
                this.notification.add("Sincronización parcial con el Gateway (Consola de Control).", {
                    type: "info",
                    sticky: false,
                });
            }
        } catch (e) {
            this.notification.add("Sincronización parcial en modo offline local.", {
                type: "info",
                sticky: false,
            });
        }
